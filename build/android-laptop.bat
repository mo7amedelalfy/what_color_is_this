...
:build-apk
	...
	call "%AIR_SDK%\bin\adt" -package -target apk-debug -listen -storetype pkcs12 -keystore %PROJECT_PATH%\build\android\cert.p12 -storepass 12345 %PROJECT_PATH%\bin-android\%FILENAME%.apk %PROJECT_PATH%\src\%FILENAME%-app.xml %FILENAME%.swf assets
